...
export default function Podium({ entries }: PodiumProps) {
  const podium = tiers.map((tier, index) => ({ tier, entry: entries[index] ?? null }));
    ...
    <section className="relative z-10 mt-16 w-full rounded-[40px] border border-[#2d2011] bg-gradient-to-b from-[#160d06] via-[#0c0704] to-[#080502] p-10 shadow-aurora backdrop-blur-2xl" data-gradient-border>
      <div className="grid gap-6 md:grid-cols-3">
        {podium.map(({ tier, entry }) => (
          ...
          >
            <p className="text-xs uppercase tracking-[0.45em] text-[#b0894d]">Rank {entry?.rank ?? tier.rank}</p>
            <h3 className="mt-2 font-serifDisplay text-2xl text-dusk-50">
              {entry?.team ?? "Awaiting champion"}
            </h3>
            {!entry && <p className="text-sm text-dusk-100/70">Slot opens soon</p>}

            <div className="relative mt-4 flex h-48 w-full items-center justify-center">
              <Image
                src={tier.trophy}
                alt={`${entry?.team ?? "placeholder"} trophy`}
                fill
                sizes="(min-width: 1024px) 20vw, 60vw"
                className={clsx("object-contain", !entry && "opacity-40")}
            ...
            <div className={clsx("mt-6 w-full rounded-3xl bg-gradient-to-b p-4", tier.gradient)}>
              ...
              <p className="text-4xl font-serifDisplay text-coal">{entry ? entry.score : "—"}</p>
              <div className="mt-4 flex items-center justify-center text-xs text-coal/80">
                <span>Solves {entry ? entry.solved : "—"}</span>
              </div>
            </div>
          </motion.div>
        ))}
